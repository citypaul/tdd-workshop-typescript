...
test("the input is not described by the result region before any submission", async () => {
  const screen = await render(<App />);
  ...
  await expect
    .element(
      screen.getByRole("textbox", {
        description: "Visa cards must be 16 digits",
      }),
    )
    .toBeVisible();
});
...
test("a successful validation clears the input error description", async () => {
  const screen = await submitCardNumber("4111111111111111");
